...
// RegisterDeliveryMethodsHandlerServer registers the http handlers for service DeliveryMethods to "mux".
// UnaryRPC     :call DeliveryMethodsServer directly.
...
// RegisterDeliveryMethodsHandlerFromEndpoint is same as RegisterDeliveryMethodsHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterDeliveryMethodsHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterDeliveryMethodsHandler(ctx, mux, conn)
}

// RegisterDeliveryMethodsHandler registers the http handlers for service DeliveryMethods to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterDeliveryMethodsHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterDeliveryMethodsHandlerClient(ctx, mux, NewDeliveryMethodsClient(conn))
}

// RegisterDeliveryMethodsHandlerClient registers the http handlers for service DeliveryMethods
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "DeliveryMethodsClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "DeliveryMethodsClient"
...
var (
	...
	pattern_DeliveryMethods_Update_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2}, []string{"v1", "delivery-methods", "id"}, ""))

	pattern_DeliveryMethods_Delete_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2}, []string{"v1", "delivery-methods", "id"}, ""))
)
